...
class PublicController
{
    public function displayIndex()
    {
        ...
        foreach ($galleries as $gallery) {
            $gallery->url = Config::get('galleries::base_url') . '/' . $gallery->id . '/' . Str::slug($gallery->title);
            $gallery->photo_url = Config::get('galleries::base_url') . '/photo/medium/' . $gallery->photo_id;
        }
    ...
    public function displayGallery($id, $slug)
    {
        ...
        if ($slug !== Str::slug($gallery->title)) {
            return Response::redirect(Config::get('galleries::base_url') . '/' . $gallery->id . '/' . Str::slug($gallery->title));
        }
        ...
        foreach ($photos as $photo) {
            $photo->xlarge_url = Config::get('galleries::base_url') . '/photo/xlarge/' . $photo->id;
            $photo->small_url = Config::get('galleries::base_url') . '/photo/small/' . $photo->id;
        }
        ...
        foreach ($other_galleries as $other_gallery) {
            $other_gallery->url = Config::get('galleries::base_url') . '/' . $other_gallery->id . '/' . Str::slug($other_gallery->title);
            $other_gallery->photo_url = Config::get('galleries::base_url') . '/photo/small/' . $other_gallery->photo_id;
        }
